...
void MainWindow::on_pushBDelClients_clicked()
{
    ...
    if (!Base.checkAgree(ui->lineEdit->text())){
        ...
        QMessageBox::warning(this, "Ошибка", "На этого клиента оформлен договор, удаление невозможно!");
    }
    if (f){
...
void MainWindow::on_pushBDelFilial_clicked()
{
    ...
    f = Base.remSubsid(ui->lineEdit_2->text());
        ...
        QMessageBox::warning(this, "Ошибка", "В этом филиале оформлен договор, удаление невозможно!");
    }
    if (f){
...
void MainWindow::on_pushBDelAgr_clicked()
{
    ...
    if (f){
        QMessageBox::warning(this, "Ошибка", "Нет договора с таким номером!");
    }
