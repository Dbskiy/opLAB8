...
void AddAgree::on_pushButton_clicked()
{
    ...
    if(ui->lineEdit->text() == "" || ui->lineEdit_2->text() == "" || ui->lineEdit_3->text() == "" || ui->lineEdit_5->text() == "" || ui->lineEdit_6->text() == ""){
        QMessageBox::warning(this, "Ошибка", "Некоторые поля формы остались пусты!");
        return;
    }
    ...
    if(flag == 100){
        QMessageBox::warning(this, "Ошибка", "Такой номер договора уже зарегистрирован!");
    }
    else if (flag > 0){
        QMessageBox::warning(this, "Ошибка", "Нет филиала с таким кодом!");
        //filial
    }
    else if(flag == -1){
        QMessageBox::warning(this, "Ошибка", "Нет клиента с таким номером паспорта и нет филиала с таким кодом!");
    }
    else if(flag < 0){
        QMessageBox::warning(this, "Ошибка", "Нет клиента с таким номером паспорта!");
    }
    else{
